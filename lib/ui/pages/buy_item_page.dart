...
class BuyItemPage extends StatelessWidget {
  BuyItemPage({Key? key}) : super(key: key);
            ...
            body: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    "Доступные лоты:\nАпи использует режим \'Песочница\' и не поддерживает количество лотов",
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        if (_portfolioController.balance <
                            _buyItemController.counter.value *
                                _buyItemController.item.value!.faceValue) {
                          Get.snackbar(
                            "Ошибка",
                        ...
                        try {
                          if (_buyItemController.type == "stock") {
                            await _portfolioController.buyOrSellStock(
                                ...
                                _buyItemController.itemQuantity +
                                    _buyItemController.counter.value,
                                (_portfolioController.balance -
                                    _buyItemController.counter.value *
                                        _buyItemController.item.value!.faceValue)
                            ...
                            await _portfolioController.buyOrSellCurrency(
                              (_buyItemController.item.value as CurrencyModel)
                                  .ticker,
                              _buyItemController.itemQuantity +
                                  _buyItemController.counter.value,
                              (_portfolioController.balance -
                      ...
                      child: Text(
                          "Купить ${_buyItemController.counter.value * _buyItemController.item.value!.faceValue}₽"),
                    ),
                  ),
                  const SizedBox(
                    height: 20,
